...
async def directory_with_file(
    fs_directory: FSDirectory, template: Template, is_overwrite: bool = False
) -> FSDirectory:
    """
    ...
    Args:
        fs_directory: File system directory.
        template: Template.
        is_overwrite: Whether to overwrite file or not.
    ...
    is_write = (
        True
        if is_overwrite
        else not await is_directory_with_file(fs_directory, template.template_file)
    )
    return (
        FSDirectory(
            fs_directory.path,
...
async def container_with_file(
    container: dagger.Container, template: Template, is_overwrite: bool = False
) -> dagger.Container:
    """
    ...
    is_write = (
        True
        if is_overwrite
        else not await is_container_with_file(container, template.template_file)
    )
    return (
