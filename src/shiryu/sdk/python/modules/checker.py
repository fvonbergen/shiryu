...
class CheckerInit(PythonModuleInit):
    ...
    async def _module_init(
        ...
        """
        _sdk_env = await super(CheckerInit, cls)._module_init(
    ...
    def _github_actions_workflows(
        ...
        """
        github_actions_workflows = super(CheckerInit, cls)._github_actions_workflows(
    ...
    def _gitlab_jobs_stages(
        ...
        """
        gitlab_jobs_stages = super(CheckerInit, cls)._gitlab_jobs_stages(
            dagger_version, shiryu_version
        )
...
class Checker(PythonModule, CheckerInit):
    ...
    async def __pipeline(
        ...
        # mypy needs files inside the source code folders.
        config = ConfigParser()
        ...
        for source_code_folder in source_code_files_folders:
            dummy_py_output_path = cls._container_project_path() / source_code_folder
            source_code_files = sorted(
                await container.directory(str(dummy_py_output_path)).glob("**/*.py")
            )
            if len(source_code_files) == 0:
                dummy_py_template_file = TemplateFile(
                    Path("mypy_dummy.py"), dummy_py_output_path
                )
                dummy_py_template_mapping: Mapping = {}
                dummy_py_template = Template(
                    PYTHON_JINJA_ENVIRONMENT,
                    dummy_py_template_file,
                    dummy_py_template_mapping,
                )
                container = await container_with_file(
                    container, dummy_py_template, False
                )
    ...
    async def check(
        ...
        await self.__pipeline(project, platform)
