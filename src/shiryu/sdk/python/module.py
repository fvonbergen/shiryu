...
class PythonModuleInit(SDKModuleInit):
    ...
    @final
    @classmethod
    def __py_typed_template_file(cls, project_name: ProjectNameType) -> TemplateFile:
        """
        Get the py.typed template file.

        Args:
            project_name: Project name.

        Returns:
            The py.typed tamplate file.
        """
        return TemplateFile(
            Path("py.typed"), cls._container_project_source_path() / project_name
        )
    ...
    async def _module_init(
        ...
        """
        _sdk_env = await super(PythonModuleInit, cls)._module_init(
        ...
        py_typed_template = Template(
            ...
            cls.__py_typed_template_file(project_properties.name),
...
class PythonModule(SDKModule, PythonModuleInit):
    ...
    def _base_container(cls, platform: PlatformType) -> dagger.Container:
        ...
        Returns:
            A base container.
        """
        container = super(PythonModule, cls)._base_container(platform)
    ...
    async def _sdk_module_init(
        ...
        is_container_with_file_pylock_toml = (
            ...
            in await container.directory(str(cls._container_project_path())).entries()
        )
